...
func TestRead(t *testing.T) {
	...
	getBlockBytes := func(index int64, data []byte) []byte {
		blockStub := blockconverter.Block{
			Index: index,
			Data:  data,
		}
		...
		err := blockStub.WriteToWithBlockInfo(w)
		if err != nil {
			log.Fatal(err)
		}
		w.Flush()
	...
	r := bytes.NewReader(getBlockBytes(index, data))
	d := NewCommand()
...
func TestDecompress(t *testing.T) {
	t.Run("should decompress data", func(t *testing.T) {
		...
		// act
		result := d.Convert(job)

		block := <-result

		// assert
		assert.Equal(t, index, block.Index)
		assert.Equal(t, text, string(block.Data))
	})
}
...
func TestWrite(t *testing.T) {
	...
	go func() {
		index := int64(0)
		for _, data := range sp {
			b := blockconverter.Block{Index: index, Data: data}
			size += len(b.Data)
			job <- b
			index++
		}
